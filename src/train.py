...
def train_one_epoch(train_dataloader, model, optimizer, loss):
    ...
    if torch.cuda.is_available():
        # DONE: transfer the model to the GPU
        # HINT: use .cuda()
        model.cuda()

    # DONE: set the model to training mode
    model.train()
    train_loss = 0.0
    ...
    for batch_idx, (data, target) in tqdm(
        ...
        output  = model(data) # DONE
        # 3. calculate the loss
        loss_value  = loss(output, target) # DONE
        # 4. backward pass: compute gradient of the loss with respect to model parameters
        loss_value.backward() # DONE
...
def optimize(data_loaders, model, optimizer, loss, n_epochs, save_path, interactive_tracking=False):
    ...
    #scheduler  = torch.optim.lr_scheduler.ExponentialLR(optimizer, gamma=0.9) # DONE
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode = "min", threshold=0.01, patience=4)
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        if valid_loss_min is None or (
            ...
            torch.save(model.state_dict(), save_path) # DONE

            valid_loss_min = valid_loss

        # Update learning rate, i.e., make a step in the learning rate scheduler
        scheduler.step(valid_loss) #DONE
...
def one_epoch_test(test_dataloader, model, loss):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (data, target) in tqdm(
            ...
            # 1. forward pass: compute predicted outputs by passing inputs to the model
            logits  = model(data) # DONE
            # 2. calculate the loss
            loss_value  = loss(logits, target) # DONE
            ...
            pred  = logits.data.max(1, keepdim=True)[1] # DONE
            """#torch.max(logits, 1).indices.squeeze() #torch.max(logits,1).data.squeeze()"""
